...
9. Модуль для считывания финансовых операций из CSV и XLSX файлов file_readers.py 
содержит функции для считывание финансовых операций из CSV- и XLSX-файлов.
Для проверки работы модуля file_readers.py добавьте в конец файла:


```
if __name__ == "__main__":
     # Пример использования функции для чтения CSV
     csv_data = read_transactions_from_csv('../data/transactions.csv')
     print(f"Загружено из CSV: {len(csv_data)} транзакций")

     # Пример использования функции для чтения Excel
     excel_data = read_transactions_from_excel('../data/transactions_excel.xlsx')
     print(f"Загружено из Excel: {len(excel_data)} транзакций")

     # Пример обработки ошибки
     error_data = read_transactions_from_csv('несуществующий_файл.csv')
     print(f"Обработка ошибки: {len(error_data)} транзакций")
```

10. Модуль main.py является точкой входа в приложение и содержит основной код для запуска приложения.
Для проверки работы модуля main.py добавьте в конец файла:
```
if __name__ == "__main__":
    main()
```
11. Модуль logic.py содержит функцию, которая принимает список словарей с
